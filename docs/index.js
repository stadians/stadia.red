...
const Home = ({ games }) => (render("main", { style: `
  ...
  ` },
    ...
    window.chrome?.runtime?.id && (render("button", { onclick: async () => (await import("./index/spider.js")).spider() }, "\uD83D\uDD77\uFE0Fspider stadia")),
    render("section", null, games.map((game) => (render(Game, Object.assign({}, game)))))));
const Game = ({ name, description, type, sku, app, addons, bundles }) => (render("section", null,
    render("h2", null,
        name,
        " [",
        type === "game" ? (render("span", null,
            render("a", { href: `http://stadia.google.com/player/${app}` }, "launch game"),
            " or")) : (type),
        " ",
        render("a", { href: `https://stadia.google.com/store/details/${app}/sku/${sku}` }, "view in store"),
        "]"),
    render("p", null, description),
    addons.map((addon) => (render("section", null,
        render("h3", null,
            render("span", { style: "font-weight: normal;" }, "\u2795"),
            " ",
            addon.name,
            " [",
            render("a", { href: `https://stadia.google.com/store/details/${app}/sku/${addon.sku}` }, "view in store"),
            "]"),
        render("p", null, description)))),
    bundles.map((bundle) => (render("section", null,
        render("h3", null,
            render("span", { style: "font-weight: normal;" }, "\uD83D\uDCE6"),
            " ",
            bundle.name,
            " [",
            render("a", { href: `https://stadia.google.com/store/details/${app}/sku/${bundle.sku}` }, "view in store"),
            "]"),
        render("p", null, description))))));
(async () => {
